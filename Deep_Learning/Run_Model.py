...
from Return_Train_Validation_Generators import return_generators, get_layers_dict_atrous, return_dictionary,\
    return_dictionary_best_4layer, return_dictionary_new, return_base_dict
...
def train_model(epochs=50,run_best=False, save_a_model=False, path_extension='Single_Images3D_1mm'):
    ...
    num_cycles = 25
    step_size = len(train_generator)
    base_dict = return_base_dict(step_size_factor=step_size_factor, step_size_add=0, cube_size=cube_size)
    ...
    for iteration in range(3):
        if run_best:
            ...
            overall_dictionary = return_dictionary_new(base_dict)
